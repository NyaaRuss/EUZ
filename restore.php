...
if (isset($_GET['id'])) {
    ...
    $sqlSelect = "SELECT * FROM past_members WHERE id='$id'";
    $result = mysqli_query($conn, $sqlSelect);
    $deletedMember = mysqli_fetch_assoc($result);

    // Insert the deleted member into the past list table
    $sqlInsert = "INSERT INTO members (id, Surname, First_name, EmpNo, NatRegNo, DOB, AppDate, StationDescription, Descriptions, Province, Department, Statcode, Amount) 
    ...
    if(mysqli_query($conn, $sqlInsert)) {
        ...
        $sqlDelete = "DELETE FROM past_members WHERE id='$id'";
        if(mysqli_query($conn, $sqlDelete)) {
            $_SESSION["delete"] = "Member moved to restored list successfully!";
        ...
        echo "Error inserting back to members table: " . mysqli_error($conn);
    }
} else {
